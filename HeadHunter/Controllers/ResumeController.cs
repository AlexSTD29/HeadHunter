...
namespace HeadHunter.Controllers
{
    public class ResumeController : Controller
    {
        ...
        {
            var resumes = _context.Resumes.Where(v => v.Status == PublishStatus.InPublic).OrderByDescending(r=>r.RefreshDate);
            return View(resumes);
        }
        ...
        public IActionResult ChangeStatus(int resumeId)
        {
            ...
            if (resume.Status == PublishStatus.InPublic)
                resume.Status = PublishStatus.Secure;
            else
                resume.Status = PublishStatus.InPublic;
